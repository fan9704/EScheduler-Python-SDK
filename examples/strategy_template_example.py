...
async def demonstrate_strategy_templates():
    ...
    async with ESchedulerSDK(
        ...
        print("=== 任務範本 (Strategy Template) 使用範例 ===")
        print("此範例將展示如何使用預定義的範本快速創建不同類型的任務.\n")
        ...
        try:
            ...
            print("1. 創建 HTTP 任務 (使用 HttpTaskTemplate)")
            ...
            print(f"✅ HTTP 任務創建成功，ID: {http_task.id}")
            print(f"   目標 ARN: {http_task.target_arn}\n")

            # 2. 使用 WebhookTaskTemplate 創建 Webhook 任務
            print("2. 創建 Webhook 任務 (使用 WebhookTaskTemplate)")
            ...
            print(f"✅ Webhook 任務創建成功，ID: {webhook_task.id}")
            print(f"   目標 ARN: {webhook_task.target_arn}\n")

            # 3. 使用 RabbitMQTaskTemplate 創建 RabbitMQ 任務
            print("3. 創建 RabbitMQ 任務 (使用 RabbitMQTaskTemplate)")
            ...
            print(f"✅ RabbitMQ 任務創建成功，ID: {rabbitmq_task.id}")
            print(f"   目標 ARN (Routing Key): {rabbitmq_task.target_arn}\n")

            # 4. 使用 EmailTaskTemplate 創建直接發送的 Email 任務
            print("4. 創建 Email 任務 (使用 EmailTaskTemplate - 直接發送)")
            ...
            print(f"✅ Email 任務創建成功，ID: {email_task.id}")
            print(f"   目標 ARN (Subject): {email_task.target_arn}\n")

            # 5. 創建使用郵件範本的 Email 任務
            print("5. 創建使用郵件範本的 Email 任務 (使用 EmailTaskTemplate with Template)")
            ...
            print(f"   ✅ 郵件範本創建成功，ID: {created_template.id}")

            # 5.2 使用範本創建任務
            print("   5.2. 使用範本 ID 創建排程任務...")
            ...
            print(f"✅ 使用範本的 Email 任務創建成功，ID: {email_task_2.id}")
            print(f"   目標 ARN: {email_task_2.target_arn}\n")

            print("=== 所有範本任務創建成功 ===")
            print("創建的任務列表:")
            for task in created_tasks:
                print(f"  - {task.name} (ID: {task.id}, 類型: {task.target_type})")

        except ESchedulerError as e:
            print(f"❌ 發生 API 錯誤: {e}")
            print(f"   狀態碼: {e.status_code}")
            print(f"   回應: {e.response_data}")
            ...
            if created_tasks:
                ...
                for task in created_tasks:
                    try:
                        ...
                        print(f"✅ 任務 {task.name} (ID: {task.id}) 已成功刪除。")
                    except ESchedulerError as e:
                        print(f"❌ 刪除任務 {task.name} (ID: {task.id}) 失敗: {e}")
